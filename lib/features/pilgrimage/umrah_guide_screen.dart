// lib/features/pilgrimage/umrah_guide_screen.dart
...
// ─────────────────────────────────────────────────────────────────────────────
// UI metadata per step (icon + accent) — UI concern, stays local
// ─────────────────────────────────────────────────────────────────────────────

const _stepMeta = [
  (icon: Icons.self_improvement_rounded, accent: AppColors.emeraldBright),
  (icon: Icons.rotate_right_rounded,     accent: AppColors.gold),
  (icon: Icons.swap_horiz_rounded,       accent: AppColors.goldBright),
  (icon: Icons.content_cut_rounded,      accent: AppColors.emeraldLight),
];
...
class _UmrahGuideScreenState extends State<UmrahGuideScreen> {
  List<PilgrimageStepModel> _steps = ContentRepository.instance.bundledUmrahSteps;
  ...
  Future<void> _loadProgress(List<PilgrimageStepModel> steps) async {
    ...
    for (final s in steps) {
      map[s.id] = prefs.getBool('umrah_done_${s.id}') ?? false;
    }
  ...
  Future<void> _refresh() async {
    try {
      final result = await ContentRepository.instance.getUmrahSteps();
  ...
  Future<void> _toggleStep(String id) async {
    ...
    await prefs.setBool('umrah_done_$id', newVal);
                ...
                child: DecoratedBox(
                  decoration: BoxDecoration(
                    gradient: RadialGradient(
                      colors: [AppColors.emeraldBright.withValues(alpha: 0.10), Colors.transparent],
                    ),
                  ),
                ),
              ),
            ...
            child: CustomScrollView(
              slivers: [
                ...
                SliverAppBar(
                  ...
                  leading: IconButton(
                    icon: const Icon(Icons.arrow_back_ios_new_rounded, color: AppColors.emeraldBright),
                    onPressed: () { HapticFeedback.lightImpact(); Navigator.of(context).pop(); },
                  ),
                  title: Text(
                    t.umrahScreenTitle,
                    style: GoogleFonts.cinzel(
                      color: AppColors.emeraldBright, fontWeight: FontWeight.w700,
                  ...
                  child: Padding(
                    ...
                    child: Text(
                      ...
                      style: const TextStyle(
                        color: AppColors.emeraldBright, fontSize: 11,
                ...
                SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, i) {
                      final step = _steps[i];
                      final meta = i < _stepMeta.length ? _stepMeta[i] : _stepMeta.last;
...
class _ProgressChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = allDone ? AppColors.emeraldBright : AppColors.emeraldBright;
    return Container(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: AppColors.emeraldBright.withValues(alpha: 0.38), width: 1.2),
      ),
      child: Column(
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              color: AppColors.emeraldBright.withValues(alpha: 0.12),
              border: Border.all(color: AppColors.emeraldBright.withValues(alpha: 0.40)),
            ),
          ...
          Text(
            t.umrahCardTitle,
            style: GoogleFonts.cinzel(
              color: AppColors.emeraldBright, fontSize: 26,
          ...
          Text(
            t.umrahCardSubtitle,
            style: TextStyle(
              color: AppColors.emeraldBright.withValues(alpha: 0.70),
              fontSize: 12, fontWeight: FontWeight.w600, letterSpacing: 1.0,
            ),
          ),
          const SizedBox(height: 16),
          Container(height: 1, color: AppColors.emeraldBright.withValues(alpha: 0.20)),
          const SizedBox(height: 16),
          Text(
            t.umrahIntro,
            ...
            Row(
              children: [
                Expanded(
                  child: ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      value: progress,
                      minHeight: 6,
                      backgroundColor: AppColors.emeraldBright.withValues(alpha: 0.15),
                      valueColor: AlwaysStoppedAnimation<Color>(AppColors.emeraldBright),
                    ),
                  ),
                ),
                ...
                Text(
                  ...
                  style: TextStyle(
                    color: AppColors.emeraldBright.withValues(alpha: 0.85),
            ...
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: accent.withValues(alpha: _expanded ? 0.45 : (completed ? 0.35 : 0.20)),
                  ),
                ),
                child: ClipRRect(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: GoogleFonts.amiri(color: accent, fontSize: 18, height: 1.8),
          ),
...
// ─────────────────────────────────────────────────────────────────────────────
// Global Shia note card at bottom
// ─────────────────────────────────────────────────────────────────────────────
